...
namespace ProjectT.UI
{
    ...
    public class HoldGaugeUI : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            _coordinate = GetComponent<Coordinate.Coordinate>();
            if (_coordinate == null)
        ...
        void OnHoldStarted()
        {
            ...
            if (playerInteract.DetectZone.CurrentTarget != _coordinate) return;
        ...
        void OnHoldEnded()
        {
            // Started 를 우리가 무시한 경우엔 _isHolding 이 false 라서 아래 HideGauge 만 no-op 로 처리됨.
            _isHolding = false;
            _elapsed = 0f;
            HideGauge();
        }
        ...
        void UpdateGaugeValue()
        {
            ...
            float t = holdDuration > 0f ? Mathf.Clamp01(_elapsed / holdDuration) : 1f;
            slider.value = t;
        }
